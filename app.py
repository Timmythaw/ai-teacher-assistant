...
if st.session_state["credentials"] is None:
    if st.button("🔑 Login with Google"):
        try:
            ...
            creds = flow.run_local_server(port=8081, browser="chrome")
            st.session_state["credentials"] = creds.to_json()
            st.success("✅ Google account connected. Please refresh the page.")
        except OSError as e:
            if "Address already in use" in str(e):
                st.warning("Port 8081 is busy. Trying to use existing credentials...")
                try:
                    ...
                    st.error("Port 8081 is busy and no valid credentials found. Please try again later.")
            else:
    ...
    try:
        ...
        if st.button("Suggest Lesson Times"):
            ...
            with st.spinner("AI is analyzing your calendar and suggesting times..."):
                ...
                suggestion = timetable_agent(user_request, openai_key)
